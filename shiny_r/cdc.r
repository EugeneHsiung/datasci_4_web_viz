...
ui <- fluidPage(
  titlePanel("Diagnosed Cancer Age-adjusted Prevalence in CA by County"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  df_cancer <- reactive({
    data <- df()
    filter(data, MeasureId == "CANCER", Data_Value_Type == "Age-adjusted prevalence")
  })
  ...
  output$barPlot <- renderPlot({
    ...
    ggplot() +
      ...
      geom_hline(aes(yintercept = avg_value), linetype = "dashed", color = "azure") +
      labs(title = 'Diagnosed Cancer Age-adjusted Prevalence',
           y = 'Data Value (Age-adjusted prevalence) - Percent',
           x = 'Location (County)') +
      theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
      ylim(0, 30) +
      scale_fill_manual(values = c("gold", "slategray"))
  })
